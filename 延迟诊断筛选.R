...
charls_wide_1 <- charls_wide_filtered %>%
  ...
  mutate(
    outcome = ifelse(dementia_age < 70, 1, 0),
...
ukb_selected <- ukb %>%
  ...
  mutate(
    time = if_else(
      EOD == 1,
      # EOD == 1：EODtime - Year.of.birth - Age.at.recruitment
      EODtime - Year.of.birth - Age.at.recruitment,
      # EOD == 0：2024 - Year.of.birth，截断到 65，再减去 Age.at.recruitment
      pmin(2024 - Year.of.birth, 65) - Age.at.recruitment
    )
  ) %>%
  # 2. 删掉不需要的两列
  select(-EODtime, -Year.of.birth) %>%
  # 3. 把 sample, EOD, time 三列移到最前面，其它列保持原顺序
...
# 分别抽取 70% 作为训练集
train_0_indices <- sample(1:nrow(data_0), size = 0.7 * nrow(data_0))
train_1_indices <- sample(1:nrow(data_1), size = 0.7 * nrow(data_1))
...
# group = 0：保留第 1–3 列，其他列进行插补
# 训练集
imp_train_0 <- mice(train_0[, -c(1, 2, 3)], m = 5, method = 'pmm', seed = 123)
train_0_imputed <- complete(imp_train_0, 1)
train_0_final <- cbind(train_0[, 1:3], train_0_imputed)

# 测试集
imp_test_0 <- mice(test_0[, -c(1, 2, 3)], m = 5, method = 'pmm', seed = 123)
test_0_imputed <- complete(imp_test_0, 1)
test_0_final <- cbind(test_0[, 1:3], test_0_imputed)

# group = 1：同样保留第 1–3 列，其他列进行 m = 5 次插补
# 训练集
imp_train_1 <- mice(train_1[, -c(1, 2, 3)], m = 5, method = 'pmm', seed = 123)
train_1_imputed <- complete(imp_train_1, 1)
train_1_final <- cbind(train_1[, 1:3], train_1_imputed)

# 测试集
imp_test_1 <- mice(test_1[, -c(1, 2, 3)], m = 5, method = 'pmm', seed = 123)
test_1_imputed <- complete(imp_test_1, 1)
test_1_final <- cbind(test_1[, 1:3], test_1_imputed)



# 合并两组数据
train_full <- rbind(train_0_final, train_1_final)
test_full <- rbind(test_0_final, test_1_final)
...
# 分别对训练集和验证集移除异常值
train_clean <- remove_outliers(train_full, columns_to_check)
test_clean <- remove_outliers(test_full, columns_to_check)

write.csv(train_clean[, -which(colnames(train_clean) == "time")], "train.csv", row.names = FALSE)
write.csv(train_clean[, 1:3], "train_ID.csv", row.names = FALSE)
write.csv(test_clean[, -which(colnames(test_clean) == "time")], "test.csv", row.names = FALSE)
write.csv(test_clean[, 1:3], "test_ID.csv", row.names = FALSE)
